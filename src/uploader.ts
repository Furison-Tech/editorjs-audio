...
import type { UploadResponseFormat, AudioConfig } from './types/types';
...
export default class Uploader {
   ...
   * @param params.config - audio tool config
  ...
  public uploadSelectedFile({ onPreview }: UploadOptions): void {
    ...
    if (this.config.uploader && typeof this.config.uploader.uploadByFile === 'function') {
      ...
      upload = ajax.selectFiles({ accept: this.config.types ?? 'audio/wav,audio/mpeg,audio/ogg' }).then((files: File[]) => {
  ...
  public uploadByFile(file: Blob, { onPreview }: UploadOptions): void {
    ...
    if (this.config.uploader && typeof this.config.uploader.uploadByFile === 'function') {
      ...
      formData.append(this.config.field ?? 'audio', file);
